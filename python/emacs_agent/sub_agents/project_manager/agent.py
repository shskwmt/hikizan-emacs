...
7.  **Read and Propagate Project Context (AGENTS.md and .dir-locals.el)**: When you are in a project or have set a directory, check if an `AGENTS.md` file or a `.dir-locals.el` file exists in the project root (`default-directory`). Read their contents if they exist and include a concise summary of the most important guidelines (like project philosophy and naming conventions) in your response when transferring control back to `emacs_agent`. This information is crucial for all agents to understand the project-specific agent instructions. Avoid including the full content of large files; prioritize summarizing for token optimization.
</INSTRUCTIONS>
